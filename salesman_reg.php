...
$query_reg = "select * from salesman where username='$user_reg' or email = '$email_reg' ";
...
if($pass2_reg!=$pass1_reg){
	echo "<script>					
		...
		window.location.href='salesman_login1.php';
		</script>";
}else{
	if($num_reg >=1){
		echo "<script>					
		...
		$query_reg = "insert into salesman (username,email,password) values ('$user_reg','$email_reg','$pass1_reg')";
